...
from mu.interface.dialogs import (ModeSelector, AdminDialog, ExampleSelector, FindReplaceDialog,
                                  PackageDialog)
...
class ButtonBar(QToolBar):
    ...
    def change_mode(self, mode):
        ...
        self.addAction(name="example", display_name=_("Example"),
                       tool_text=_("Load Example Python script."))
        self.addSeparator()
...
class Window(QMainWindow):
    ...
    title = _("Mu {} by DFRobot Team").format(__version__)
    icon = "icon"
    timer = None
    usb_checker = None
    serial = None
    ...
    def select_example(self, examples):
        """
        Display a new window for example
        """
        example_select = ExampleSelector()
        example_select.setup(examples)
        example_select.exec()
        try:
            return example_select.get_example()
