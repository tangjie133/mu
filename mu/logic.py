...
class Editor:
    ...
    def setup(self, modes):
        ...
        # riven, copy libs for mu-editor
        # maqueen_path = os.path.join(wd, 'maqueen.py')
        # if not os.path.exists(maqueen_path):
        #     shutil.copy(path('maqueen.py', './'), maqueen_path)
        huskylens_path = os.path.join(wd, 'huskylens.py')
        if not os.path.exists(huskylens_path):
            shutil.copy(path('huskylens.py', './'), huskylens_path)
    ...
    def load_example(self):
        logger.info('Showing available modes: {}'.format(
            list(self.modes.keys())))
        examples = {}
        #logger.info(self.modes[self.mode])
        if hasattr(self.modes[self.mode], 'examples'):
            examples = self.modes[self.mode].examples
            logger.info("1")
        example = self._view.select_example(examples)
        logger.info("selected example {}".format(example))
        if example:
            self._view.add_tab(example, examples[example]['code'], self.modes[self.mode].api(), NEWLINE)
    ...
    def change_mode(self, mode):
        ...
        button_bar.connect("example", self.load_example, "Ctrl+E")
